...
struct addrinfo * packageConnection(char * input_addr, char * porto);
 ...
 * HAND SHAKE 
 * **********************************************/
void handshake(int sockfd, char * clienUserName, char *serverAddress) {
	int sendIt = send(sockfd, chatHandle, strlen(clientUserName), 0);
